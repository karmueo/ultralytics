...
def preprocess_image(image_path, imgsz=640):
    ...
    if isinstance(imgsz, (list, tuple)):
        target_h = imgsz[0]  # 目标高（中文注释：模型输入高）
        target_w = imgsz[1]  # 目标宽（中文注释：模型输入宽）
    else:
        target_h = imgsz  # 目标高（中文注释：模型输入高）
        target_w = imgsz  # 目标宽（中文注释：模型输入宽）
...
def scale_boxes(boxes, original_size, model_size):
    ...
    if isinstance(model_size, (list, tuple)):
        model_h = model_size[0]  # 模型高（中文注释：模型输入高）
        model_w = model_size[1]  # 模型宽（中文注释：模型输入宽）
    else:
        model_h = model_size  # 模型高（中文注释：模型输入高）
        model_w = model_size  # 模型宽（中文注释：模型输入宽）
